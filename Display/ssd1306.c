...
void ssd1306_rect(ssd1306_t *ssd, uint8_t top, uint8_t left, uint8_t width, uint8_t height, uint8_t border, bool value, bool fill) {
  for (uint8_t i = 0; i < border; i++)
  {
    for (uint8_t x = left; x < left + width; ++x) {
      ssd1306_pixel(ssd, x, top + i, value);
      ssd1306_pixel(ssd, x, top + height - 1 - i, value);
    }
    for (uint8_t y = top; y < top + height; ++y) {
      ssd1306_pixel(ssd, left + i, y, value);
      ssd1306_pixel(ssd, left + width - 1 - i, y, value);
    }
  }

  if (fill) {
    for (uint8_t x = left + border; x < left + width - border; ++x) {
      for (uint8_t y = top + border; y < top + height - border; ++y) {
        ssd1306_pixel(ssd, x, y, value);
      }
    }
  }
